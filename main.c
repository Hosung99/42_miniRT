...
t_scene	*scene_init(void)
{
	...
	// oadd(PL, plane(vec3(0.0,1.0,0.0), point3(0.0,-10.0,0.0), color3(0,0.5,0)), color3(0.5,0,0));
    // oadd(&world, object(SP, sphere(point3(0, -1000, 0), 995), color3(1, 1, 1))); // world 에 구3 추가
    // oadd(&world, object(SP, sphere(point3(2, 0, -5), 2), color3(0, 0.5, 0))); // world 에 구2 추가
	// world = object(PL, plane(vec3(0.0,-1.0,0.0), point3(0.0,-10.0,0.0), color3(0,0.5,0)), color3(0.5,0,0));
    scene->world = world;
    lights = object(LIGHT_POINT, light_point(point3(1, 0, 0), color3(1, 1, 1), 0.5), color3(0, 1, 0)); // 더미 albedo
	scene->light = lights;
	ka = 0.1;
	scene->ambient.r = ka;
	scene->ambient.g = ka;
